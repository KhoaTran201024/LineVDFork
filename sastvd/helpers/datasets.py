...
def bigvul(minimal=True, sample=False, return_raw=False, splits="default"):
    ...
    if minimal:
        try:
            ...
            print("Enter try line 144 of BigVul dataset")
            if "crossproject" in splits:
                print("Enter cross project 146")
            ...
            print("stp at line 155")
            print(len(df))
            '''print(f"TRAIN:NON VUL DATA LEN {len(df[(df.vul == 0) & (df.label =='train')])}\n")
            print(f"TRAIN:VUL DATA LEN {len(df[(df.vul == 1) & (df.label =='train')])}\n")

            print(f"TEST:NON VUL DATA LEN {len(df[(df.vul == 0 )&(df.label =='test')])}\n")
            print(f"TEST:VUL DATA LEN {len(df[(df.vul == 1)&(df.label =='test')])}\n")

            print(f"TEST:NON VUL DATA LEN {len(df[(df.vul == 0 ) & (df.label =='val')])}\n")
            print(f"TEST:VUL DATA LEN {len(df[(df.vul == 1) & (df.label =='val')])}\n")

            print(f"NON VUL DATA LEN {len(df[df.vul == 0])}\n")
            print(f"VUL DATA LEN {len(df[df.vul == 1])}\n")'''
            return df
        except Exception as E:
            print("Error line 172")
            print(E)
            pass
    print("Enter line 160")
